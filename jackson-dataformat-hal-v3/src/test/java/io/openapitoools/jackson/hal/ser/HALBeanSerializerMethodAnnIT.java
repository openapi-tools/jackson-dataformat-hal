package io.openapitoools.jackson.hal.ser;
...
public class HALBeanSerializerMethodAnnIT {
    ...
    public void testCuries() {
        ...
        assertEquals("http://other.link.url2", relative2UsingCurie.get("href").asString());
        assertFalse(relative2UsingCurie.get("templated").asBoolean());

        JsonNode relative3UsingCurie = links.get("cur2:relative3");
        assertFalse(relative3UsingCurie.isMissingNode());
        assertEquals("http://other.link.url3{?id}", relative3UsingCurie.get("href").asString());
        assertTrue(relative3UsingCurie.get("templated").asBoolean());


        JsonNode relative4UsingCurie = links.get("cur3:relative4");
        assertFalse(relative4UsingCurie.isMissingNode());
        assertEquals("/other.link.url4{?id}", relative4UsingCurie.get("href").asString());
        assertTrue(relative4UsingCurie.get("templated").asBoolean());
        ...
        assertEquals("POJO name", jsonNode.get("name").asString());
    }
    ...
    public void singleCurieTest() throws Exception {
        ...
        assertEquals("cur1", curie1.get("name").asString());
        assertEquals("http://docs.my.site/{rel}", curie1.get("href").asString());
